...
class PaymentVoucherCotnroller extends GetxController {
  bool loading = false;
  bool onsaved = false;
  ...
  Future<void> submit() async {
    ...
    if (paymentDate != "" &&
      ...
      try {
        ...
        var url = Uri.parse(Url.cabReciepts);
        if (kDebugMode) {
          print("reatching :: ${Url.cabReciepts}");
        }
        final data = {
          "type": "0",
        ...
        if (response.statusCode == 201) {
          result = CabRecieved.fromJson(json.decode(response.body));
          update();

          Get.to(() => CabResultView(
                result: result!,
              ));
        } else {
